...
module.exports = postcss.plugin('rtlcss', function (options, plugins, hooks) {
  ...
  return function (css, result) {
    ...
    const { tmplOpen, tmplClose } = context.config
    function toRtlTmpl(normalEntity, rtlEntity) {
      return `${tmplOpen}?!it.rtl${tmplClose}${normalEntity}${tmplOpen}??${tmplClose}${rtlEntity}${tmplOpen}?${tmplClose}`
    }
          ...
          case 'decl':
                      ...
                      if (expr.test(node.raws.value.raw)) {
                        ...
                        if (directive.action(node, expr, context)) {
                          if (context.config.clean) {
                            ...
                            const rtlVal = context.util.trimDirective(node.raws.value.raw)
                            if (rtlVal !== node.raws.value.value) {
                              node.value = node.raws.value.raw = toRtlTmpl(node.raws.value.value,rtlVal)
                            } else {
                              node.value = node.raws.value.raw = rtlVal //trimmed val
                            }
                          }
                          flipped++
                          // break
              ...
              return context.util.each(plugin.processors, function (directive) {
                if (node.prop.match(directive.expr)) {
                  ...
                  if (pair.prop !== node.prop || pair.value !== raw) {
                    flipped++
                    if (node.prop !== pair.prop) {
                      //Prop replacement
                      node.prop = toRtlTmpl(node.prop,pair.prop) 
                    }
                    if (node.value !== pair.value) {
                      //Prop replacement
                      node.value = toRtlTmpl(node.value,pair.value)
                    }
                  }
                  // match found, break
                  return false
            ...
            if (context.config.autoRename && !flipped && node.parent.type === 'rule' && context.util.isLastOfType(node)) {
              ...
              if (context.config.autoRenameStrict === true) {
                ...
                if (pair) {
                  pair.selector = toRtlTmpl(renamed,node.parent.selector)
                  node.parent.selector = toRtlTmpl(node.parent.selector,renamed) 
                } else {
                  toBeRenamed[node.parent.selector] = node.parent
                }
              } else if (node.parent.selector !== renamed) {
                node.parent.selector = toRtlTmpl(node.parent.selector,renamed) 
              }
            }
            break
